...
local MENU_ACCEPT = false
local MENU_CANCEL = false

---@param playlistName string
---@return table
local function getAllSongsFromPlaylist(playlistName)
...
local function setPlaylist(pageChange)
	...
	if allSongPlaylistExists then
		...
		for playlistPosition = 1, #displayName, 1 do
			if displayName[playlistPosition] ~= 'all' and displayName[playlistPosition] ~= 'favs' then
				playlist = getAllSongsFromPlaylist(displayName[playlistPosition])
				if playlist ~= nil and #playlist > 0 then
					for index, value in ipairs(playlist) do
						if value ~= '' then
							allPlaylist[#allPlaylist+1] = playlist[index]
						end
					end
				end
			end
...
function onUpdate()
	...
	MENU_ACCEPT = getPropertyFromClass('flixel.FlxG', 'keys.justPressed.ENTER') or getPropertyFromClass('flixel.FlxG', 'keys.justPressed.SPACE')
	MENU_CANCEL = getPropertyFromClass('flixel.FlxG', 'keys.justPressed.BACKSPACE') or getPropertyFromClass('flixel.FlxG', 'keys.justPressed.ESCAPE')
