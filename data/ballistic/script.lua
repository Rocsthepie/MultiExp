...
function onStartCountdown()
	if not allowCountdown and isStoryMode and not seenCutscene then
		...
		if stops == 0 then
			...
			runTimer('23', 155 / 24)
			runTimer('toe', 158 / 24)
			runTimer('flashhh', 158 / 24 + 2)
			runTimer('mbreak', 52 / 24)
			playSound('windLmao')

			playAnim('stage', 'BallisticBackgroundStart', false)
		elseif stops == 1 then
...
function onTimerCompleted(tag, _, _)
	...
	if tag == 'solja' then
    	playAnim('stage', 'BallisticBackgroundGo', false)
...
function onEndSong()
	endstops = endstops + 1
	if isStoryMode and endstops == 1 then
		runTimer('endshit', 1)
